...
#look up extension IDs in the Edge webstore if you want to supply them manually
...
if (!($extension)) {
    ...
    $string = (Get-Item -Path "HKLM:\SOFTWARE\Policies\Microsoft\Edge\ExtensionInstallForcelist").Property[-1]
    ...
    reg add HKLM\SOFTWARE\Policies\Microsoft\Edge\ExtensionInstallForceList /v $value /t REG_SZ /d $extension /f
}
